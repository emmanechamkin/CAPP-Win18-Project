...
c.execute(select avg(total_pop), avg(pct_white), avg(pct_black), avg(pct_other), avg(total_units), avg(Median), 
	avg(pct_occupied), avg(pct_vacant), avg(pct_own_occ), avg(pct_rent_occ) from census_weighted_avg_all;) 
c.fetchone()
c.execute(select min(total_pop), min(pct_white), min(pct_black), min(pct_other), min(total_units), min(Median),
 min(pct_occupied), min(pct_vacant), min(pct_own_occ), min(pct_rent_occ) from census_weighted_avg_all;) 
c.fetchone()
c.execute(select max(total_pop), max(pct_white), max(pct_black), max(pct_other), max(total_units), max(Median), 
	max(pct_occupied), max(pct_vacant), max(pct_own_occ), max(pct_rent_occ) from census_weighted_avg_all;) 
c.fetchone()
...
c.execute(select avg(case when median > -1 then median end) from census_weighted_avg_all;)
